# Runs claude-stats.ps1 on a small made-up log folder and checks the numbers.
# Works in Windows PowerShell 5.1 and PowerShell 7+.   Usage: ./tests/run-tests.ps1
$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent $PSScriptRoot
$env:NO_COLOR = '1'
$tmp = Join-Path ([IO.Path]::GetTempPath()) ('claude-stats-test-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $tmp | Out-Null
$script = Join-Path $root 'claude-stats.ps1'
...
# 2. The bash-style options of claude-stats.sh give the same answer
...
Write-Host 'OK: claude-stats.ps1'
